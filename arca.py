...
_TANGO_HEADERS = [
    ...
    'Tipo comprobante ARCA','Operación ARCA','Comprobante electrónico','CAE / CAI',
    'Fecha vencimiento CAE / CAI','Cotización','Número de documento', 'Código de tipo de documento','Neto Gravado 21','IVA 21','Total',
]
...
def csv_a_plantilla_tango(csv_bytes: bytes) -> bytes:
    """Convierte CSV ARCA → Plantilla Tango XLS (Excel 97-2003) usando xlwt."""
    import xlwt
    import csv
    import io
    from datetime import datetime
    ...
    header_style = xlwt.easyxf(
        'font: bold on; align: horiz right; pattern: pattern solid, fore_colour ice_blue;'
    )

    text_style = xlwt.easyxf('align: horiz left;')
    text_force = xlwt.easyxf('align: horiz left;', num_format_str='@')

    number_style = xlwt.easyxf('align: horiz right;', num_format_str='0.00')
    date_style = xlwt.easyxf('align: horiz left;', num_format_str='DD/MM/YYYY')
    cotiz_style = xlwt.easyxf(
    'align: horiz right;',
    num_format_str='0.00'
)
    # columnas especiales
    TEXT_C  = {0,1,2,5,6,7,8,9,10,19}   # base 0
    RIGHT_C = {24,25,26,27}
    DATE_C  = {3,4}
    ...
    # ─── HEADERS ───────────────────────────────
    for ci, h in enumerate(_TANGO_HEADERS):
        ws.write(0, ci, h, header_style)

    # ─── DATA ───────────────────────────────
    for ri, r in enumerate(rows_csv, 1):
        ...
        nombre = (
            r.get('Denominación Receptor') or
            r.get('Denominación del Receptor') or
            r.get('Denominacion Receptor') or
            r.get('Apellido y Nombre / Razón Social') or
            ''
        ).strip()

        if cod_doc in ('99', '', '0') or not nro_doc or nro_doc == '0':
        ...
        cotiz = float(to_decimal(r.get('Tipo Cambio','1')))
        if cotiz == 0:
            cotiz = 1.0
    ...
    # FECHA
            if ci in DATE_C and isinstance(val, datetime):
                ws.write(ri, ci, val, date_style)

            # TEXTO
            elif ci in TEXT_C:
                ws.write(ri, ci, str(val), text_force)

            # COTIZACIÓN (columna 25 → índice 24)
            elif ci == 24:
                if val in ('', None):
                    ws.write(ri, ci, '')
                else:
                    ws.write(ri, ci, val, cotiz_style)
            ...
            # NUMÉRICOS
                ...
                if val in ('', None):
                    ...
                    ws.write(ri, ci, val, number_style)

            # RESTO
            else:
                ws.write(ri, ci, val, text_style)

    # ─── CONFIG ───────────────────────────────
    wc = wb.add_sheet('Configuracion')

    conf1 = ['PLANTILLA','COD_MODELO_INGRESO','ID_MODELO_INGRESO','1','2','98']
    conf2 = ['Ventas','VENTAS',5,0,1,2]

    for ci,v in enumerate(conf1):
        wc.write(0, ci, v)

    for ci,v in enumerate(conf2):
        wc.write(1, ci, v)
...
if st.button("📋 GENERAR PLANTILLA TANGO", disabled=(file_csv_tango is None), key='btn_tango'):
    with st.spinner("Generando plantilla..."):
        try:
            ...
            st.download_button(
                label=f"⬇ PlantillaVentas_TANGO_{periodo_str}.xls",
                data=xlsx_bytes,
                file_name=f"PlantillaVentas_TANGO_{periodo_str}.xls",
